...
export default function Products() {
    ...
    const [perPage, setPerPage] = useState(15); // Default items per page
    useEffect(() => {
    ...
    try {
        ...
        setTotalPages(
            Math.max(1, Math.ceil(envelope.total / envelope.per_page))
        );
    ...
    // Trigger page data refetches whenever the active index shifts
    useEffect(() => {
        fetchProducts(currentPage);
    }, [currentPage]);

    // Triggers execution explicitly when search parameters are explicitly targeted
    const handleSearchSubmit = (e) => {
        e.preventDefault();
        setCurrentPage(1); // Drop query parameters index back to initial baseline page
        fetchProducts(1);
    };
    ...
    const triggerDeleteSequence = async (id) => {
        try {
            ...
            // Refresh the current structural viewport context cleanly
            fetchProducts(currentPage);
        } catch (err) {
    ...
    return (
        <>
            ...
            {/* Interactive Filters Bar Form submission triggers deep fetch filter parameters */}
            <form onSubmit={handleSearchSubmit} className="mb-5 flex items-center gap-2 max-w-md">
                ...
                <button
                    type="submit"
                    className="h-[38px] px-4 rounded-lg cursor-pointer font-medium text-sm border bg-white hover:bg-gray-50 text-gray-700"
            ...
            {loading ? (
                ...
                <div className="rounded-xl border overflow-hidden" style={{ background: "#fff", borderColor: "#E4E3E0" }}>
                            ...
                            <tbody>
                                {products.length === 0 ? (
                                    ...
                                    products.map((p) => {
                                        ...
                                        // Dynamic calculation of parent stock remaining
                    ...
                    {/* ─── SERVER SIDE PAGINATION FOOTER CONTROL PANEL ─── */}
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between p-3.5 border-t bg-[#F7F6F3]" style={{ borderColor: "#E4E3E0", fontSize: 12, color: "#6B6B7A" }}>
                            <div>
                                Showing <span className="font-medium text-[#1C1C24]">{products.length}</span> out of{" "}
                                <span className="font-medium text-[#1C1C24]">{totalItems}</span> structural ledger targets.
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <button
                                    ...
                                    className="p-1.5 rounded-lg border bg-white text-gray-700 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer transition-colors hover:bg-gray-50 flex items-center justify-center"
                                    style={{ borderColor: "#E4E3E0" }}
                                >
                                    <IconChevronLeft size={16} />
                                </button>

                                {/* Pagination Number Matrix Grid */}
                                {[...Array(totalPages)].map((_, idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="h-7 min-w-[28px] px-1.5 rounded-md font-medium text-xs cursor-pointer transition-colors border"
                                            style={{
                                                background: isSelected ? "#E8A04B" : "#fff",
                                                color: isSelected ? "#fff" : "#1C1C24",
                                                borderColor: isSelected ? "#E8A04B" : "#E4E3E0"
                                            }}
                                        >
                                            {pageNumber}
                                        </button>
